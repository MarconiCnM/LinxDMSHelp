...
from models.models import ANALISTA, SOL_ERRO, MOV_SOL, TIME, HELPER
...
def solErro(form_erro):
    ...
    if 'btn_submit_inserir' in request.form:
        ...
        if erro:
            ...
            erro = SOL_ERRO(NRO_TP=form_erro.nro_tp.data, ISSUE='N', CAMINHO_MENU=form_erro.menu_dir.data, 
                            CODIGO_MENU=form_erro.menu_cod.data, FAZENDO=form_erro.fazendo.data, FAZER=form_erro.fazer.data, 
                            PALIATIVA=form_erro.paliativa.data, DOCS=complete_name, BANCO=form_erro.db_teste.data, VERSAO=form_erro.versao.data, 
                            VERSAO_ANT=form_erro.versao_ant.data, ANALISTA_ID=current_user.id, HELPER_ID=helper.id)
    ...
    if form_erro.validate_on_submit() and 'btn_submit_iniciar' in request.form:
        ...
        erro.FAZENDO=form_erro.fazendo.data
    ...
    if form_erro.validate_on_submit() and 'btn_submit_recus' in request.form:
        ...
        if form_erro.solucao.data == '':
            ...
            return True 
    if form_erro.validate_on_submit() and 'btn_submit_encaminhar' in request.form:
        erro = SOL_ERRO.query.filter_by(
            NRO_TP=form_erro.nro_tp.data).first()
        
        if erro:
            helper = HELPER.query.filter_by(USUARIO=form_erro.helper.data).first()

            erro.HELPER_ID=helper.id
            erro.STATUS='NÃO INICIADO'

            mov = MOV_SOL(NRO_TP=form_erro.nro_tp.data, TITULO='Solicitação Encaminhada', TIPO='H', HELPER=current_user.USUARIO, ANALISTA='', RESUMO='Solicitação teve seu helper trocado', STATUS='NÃO INICIADO')
...
def solErroTime():
    result = SOL_ERRO.query.filter_by(HELPER_ID=current_user.id)

    return result

def solErroTimeGestor():
    result = SOL_ERRO.query.filter().join(ANALISTA.query.filter().join(TIME.query.filter_by(GESTOR_ID=current_user.id)))

    return result

def solErroTimeCoordenador():
    result = SOL_ERRO.query.filter().all()
...
def enviaEmailErro(tp_id, tipo):
    if tipo == 'H':
        ...
        msg = Message(
                subject = f'ISSUE Aberta - {erroID.ISSUE}',
                sender = mail_settings["MAIL_USERNAME"],
                recipients= ['apollo@linx.com.br','bravos@linx.com.br'],
                body = f'''Bom dia Pessoal,

Segue ISSUE encaminhada para correção,

*Solicitante:* {erroID.requester.USUARIO}
*Aprovador:* {erroID.finisher.USUARIO}

*Menu:*({erroID.CODIGO_MENU}) {erroID.CAMINHO_MENU}
